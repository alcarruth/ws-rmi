...
(function() {
  ...
  var Logger, RMI_Client, RMI_Connection, RMI_Object, RMI_Object_Registry, RMI_Registry_Admin, RMI_Stub, RMI_Stub_Registry, WebSocket, inspect, random_id, stacktrace, ws_rmi,
    indexOf = [].indexOf;
  ...
  RMI_Connection = class RMI_Connection {
    ...
    ready() {
      return new Promise((resolve, reject) => {
        var delay, error, max_tries, tries, waiter;
        delay = 100;
        max_tries = 100;
        tries = 1;
        try {
          this.log(`max_tries: ${max_tries}, delay: ${delay} ms...`);
          return waiter = setInterval((() => {
            if (this.ws.readyState === this.ws.OPEN) {
              clearInterval(waiter);
              return resolve(true);
            } else if (tries >= max_tries) {
              clearInterval(waiter);
              this.log("tries >= max_tries");
              return reject("tries >= max_tries");
            } else {
              return tries += 1;
            }
          }), delay);
  ...
  ws_rmi = {random_id, Logger, RMI_Client, RMI_Connection, RMI_Object, RMI_Stub, RMI_Object_Registry, RMI_Stub_Registry};
